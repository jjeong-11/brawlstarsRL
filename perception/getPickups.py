...
def find_ground_cubes(image: np.ndarray, exclude_positions=None,
    ...
    `hsv` reuses an existing HSV conversion of the frame; `bars` reuses an
    existing find_enemy_health_bars() result so the full-screen red-bar
    scan isn't repeated (the live loop runs it once per tick and shares).
