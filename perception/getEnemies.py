...
from . import username_classifier
...
def find_enemy_health_bars(image: np.ndarray, hsv: np.ndarray = None):
    ...
    `hsv` may be passed to reuse an existing HSV conversion of the frame.
    """
    height, width = image.shape[:2]
    if hsv is None:
...
def find_entities(image, player_pos=None, hsv=None, bars=None):
    ...
    `hsv` reuses an existing HSV conversion; `bars` reuses an existing
    find_enemy_health_bars() result (the live loop shares one bar scan per
    tick between this and find_ground_cubes).
    ...
    height, width = image.shape[:2]
    if bars is None:
        bars = find_enemy_health_bars(image, hsv=hsv)
    ...
    for bar_box in bars:
        ...
        # A "username" in the bottom control/HUD band is implausible: that's the
        # player's own joystick / attack+super buttons / power-cube crates, not a
        # floating enemy name. Reclassify as a (non-attackable) box. This stops the
        # bot firing at boxes when no real enemy is on screen.
        if is_enemy and (by + bh) > height * 0.80:
            is_enemy = False
        if is_enemy:
