...
def predict_proba(features: np.ndarray, path: Path = WEIGHTS_PATH) -> float:
    ...
    if _MODEL_CACHE is None:
        m = _load_model(path)
        # Convert the JSON lists to numpy arrays ONCE at load time -- this
        # runs for every bar candidate every entity tick, and rebuilding
        # four arrays per call was measurable in the live loop.
        _MODEL_CACHE = {
    ...
    m = _MODEL_CACHE
    fn = (features - m["mean"]) / m["std"]
    z = fn @ m["weights"] + m["bias"]
